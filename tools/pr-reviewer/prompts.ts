...
export const PR_ROLES: Record<string, string> = {
  "Security Reviewer": "Focus on vulnerabilities, injection attacks, authentication issues, secret exposure, unsafe data handling, and XSS/CSRF risks.",
  "Architect": "Focus on system design, separation of concerns, API contracts, component boundaries, dependency direction, and whether the change fits the broader codebase architecture.",
  "Bug Hunter": "Focus on logic errors, off-by-one mistakes, edge cases, race conditions, null/undefined handling, and incorrect assumptions.",
  "Engineering Best Practices": "Focus on error handling, logging, naming conventions, DRY violations, SOLID principles, proper use of language/framework idioms, and code that will be easy to debug in production.",
  "Maintainability & Tests": "Focus on test coverage for new code, test quality, missing edge case tests, code that will be hard to maintain or extend, unclear abstractions, and future technical debt.",
};

export const ROLE_NAMES = Object.keys(PR_ROLES);

export function assignRoles(models: string[]): Record<string, string> {
...
export function buildFileReviewPrompt(file: FileDiff, role: string): { role: string; content: string }[] {
  return [
    {
      role: "system",
      content: `You are a code reviewer with the following specialization: ${role}
...
- Be specific and actionable
- Do not flag trivially correct code
- Prioritize important issues over style nitpicks${jiraSection}`,
    },
    {
      role: "user",
      content: `Review this diff for file: ${file.path}\n\n\`\`\`diff\n${file.rawDiff}\n\`\`\``,
